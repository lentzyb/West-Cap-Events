...
export default async req => {
  try {
    ...
    let completed = 0;
    let failed = 0;
    let calendlyShown = 0, calendlySelected = 0, calendlyScheduled = 0;
    ...
    for (const registration of registrations) {
      ...
      const cal = registration.calendly || {};
      if (cal.shownAt) calendlyShown += 1;
      if (cal.timeSelectedAt) calendlySelected += 1;
      if (cal.scheduledAt) calendlyScheduled += 1;

      const eventRow = byEvent.get(slug) || { slug, name: registration.eventName || eventMap.get(slug)?.name || slug, total: 0, completed: 0, failed: 0, calendlyShown: 0, calendlySelected: 0, calendlyScheduled: 0, recruiters: {} };
      eventRow.total += 1;
      if (status === "completed") eventRow.completed += 1;
      if (status === "failed") eventRow.failed += 1;
      if (cal.shownAt) eventRow.calendlyShown += 1;
      if (cal.timeSelectedAt) eventRow.calendlySelected += 1;
      if (cal.scheduledAt) eventRow.calendlyScheduled += 1;
      ...
      const recruiterRow = byRecruiter.get(recruiterName) || { name: recruiterName, total: 0, completed: 0, failed: 0, calendlyShown: 0, calendlySelected: 0, calendlyScheduled: 0 };
      recruiterRow.total += 1;
      if (status === "completed") recruiterRow.completed += 1;
      if (status === "failed") recruiterRow.failed += 1;
      if (cal.shownAt) recruiterRow.calendlyShown += 1;
      if (cal.timeSelectedAt) recruiterRow.calendlySelected += 1;
      if (cal.scheduledAt) recruiterRow.calendlyScheduled += 1;
    ...
    return json({
      summary: { events: events.length, activeEvents: events.filter(e => !e.archived).length, registrations: registrations.length, completed, failed, calendlyShown, calendlySelected, calendlyScheduled, bookingRate: calendlyShown ? Math.round((calendlyScheduled / calendlyShown) * 100) : 0, successRate: registrations.length ? Math.round((completed / registrations.length) * 100) : 0 },
