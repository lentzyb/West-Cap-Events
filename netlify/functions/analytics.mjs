...
export default async req=>{try{if(req.method!=="POST")return json({error:"Method not allowed."},405);const body=await parseJSON(req)||{};let auth=requireAdmin(req);if(!auth.ok&&body.adminPassword){const expected=process.env.ADMIN_PASSWORD||"";if(String(body.adminPassword)===expected)auth={ok:true};}if(!auth.ok){console.warn('analytics auth rejected',{method:req.method,hasAdminHeader:Boolean(req.headers.get('x-admin-password')),hasBodyPassword:Boolean(body.adminPassword),hasConfiguredPassword:Boolean(process.env.ADMIN_PASSWORD)});return auth.response;}
 ...
 // Orientation is the conversion trigger, not a temporary reporting bucket.
 // The first time a registration uniquely matches someone in Monday's Orientation group,
 // persist that attribution in Netlify Blobs. Moving the person to Active Roster later
 // will therefore NOT remove them from historical event ROI.
 const attributionStore=store();
 for(const m of monday.matches||[]){
   if(!m.joined||!m.registration?.id)continue;
   const key=`conversion:${m.registration.id}`;
   ...
   const attribution={
     ...
     lastSeenInOrientationAt:now,
     stageAtConversion:existing?.stageAtConversion||'Orientation',
 ...
 const attributions=await listAll('conversion:');
 ...
 for(const a of attributions){
   ...
   if(er){er.joined++;er.volume+=a.volume||0;er.convertedCandidates.push({name:a.candidateName||`${r.firstName||''} ${r.lastName||''}`.trim(),mondayName:a.mondayName||'',stage:a.stageAtConversion||'Orientation',volume:a.volume||0,matchMethod:a.matchMethod||'Saved Orientation match',email:r.email||'',phone:r.phone||'',convertedAt:a.convertedAt||''})}
 }
 ...
 return json({summary:{events:events.length,activeEvents:events.filter(e=>!e.archived).length,registrations:registrations.length,completed,failed,calendlyShown,calendlySelected,calendlyScheduled,bookingRate:calendlyShown?Math.round(calendlyScheduled/calendlyShown*100):0,successRate:registrations.length?Math.round(completed/registrations.length*100):0,joined,conversionRate:registrations.length?Math.round(joined/registrations.length*1000)/10:0,totalVolume},events:eventAnalytics,recruiters:[...byRecruiter.values()].sort((a,b)=>b.total-a.total),monday:{configured:monday.configured,mondayItems:monday.mondayItems||0,matched:(monday.matches||[]).length,persistedAttributions:attributions.length,possibleMatches:(monday.possibleMatches||[]).length,diagnostics:(monday.diagnostics||[]).slice(0,25),nameSourceDiagnostics:(monday.nameSourceDiagnostics||[]).slice(0,100),error:mondayError,board:monday.board||null,detected:monday.detected||null,orientationGroup:monday.orientationGroup||null}})
