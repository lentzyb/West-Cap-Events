...
import { getBoardMeta, getMondayConfig, saveMondayConfig } from "./_monday.mjs";
...
export default async req => {
  try {
    ...
    // Monday settings are routed through the existing Events admin endpoint.
    // This deliberately reuses the exact same authenticated function path the
    // admin dashboard already uses successfully.
    const adminAction = clean(url.searchParams.get("adminAction") || "", 80);
    if (req.method === "GET" && adminAction === "monday-meta") {
      const board = await getBoardMeta();
      const config = await getMondayConfig();
      return json({ connected: true, board, config });
    }
    if (req.method === "POST" && adminAction === "monday-save") {
      const body = await parseJSON(req);
      if (!body) return json({ error: "Invalid JSON body." }, 400);
      const config = await saveMondayConfig(body);
