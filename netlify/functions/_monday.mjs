...
export async function getOrientationItems(board, columnIds=[]){
 const boardId=process.env.MONDAY_BOARD_ID; if(!boardId) throw new Error('MONDAY_BOARD_ID is not configured in Netlify.');
 const groups=(board?.groups||[]).filter(g=>!g.archived&&!g.deleted);
 const orientation=groups.find(g=>normTitle(g.title)==='orientation')||groups.find(g=>normTitle(g.title).includes('orientation'));
 if(!orientation) throw new Error('Could not find an active Monday group named Orientation.');
 // Ask Monday only for the Orientation group. This avoids reading New Request, Confirmed,
 // or the rest of a large recruiting board and keeps the request small/fast.
 const groupId=String(orientation.id).replace(/\\/g,'\\\\').replace(/"/g,'\\"');
 const first=`query($ids:[ID!]!,$limit:Int!){boards(ids:$ids){items_page(limit:$limit,query_params:{rules:[{column_id:"group",compare_value:["${groupId}"],operator:any_of}]}){cursor items{id name group{id title} column_values{id text value}}}}}`;
 let d=await mondayRequest(first,{ids:[String(boardId)],limit:100}); let page=d.boards?.[0]?.items_page; if(!page) return {items:[],orientation};
 let items=[...(page.items||[])],cursor=page.cursor;
 ...
 while(cursor){d=await mondayRequest(next,{cursor,limit:100});page=d.next_items_page;items.push(...(page?.items||[]));cursor=page?.cursor;}
...
export async function getConversionData(registrations){
 ...
 const {items,orientation}=await getOrientationItems(resolved.board,[config.emailColumnId,config.phoneColumnId,config.volumeColumnId].filter(Boolean)); const emails=new Map(),phones=new Map();
 ...
 for(const r of registrations){const e=normalizeEmail(r.email),p=normalizePhone(r.phone); const candidates=new Map(); for(const x of emails.get(e)||[])candidates.set(x.id,{item:x,email:true,phone:false}); for(const x of phones.get(p)||[]){const c=candidates.get(x.id)||{item:x,email:false,phone:false};c.phone=true;candidates.set(x.id,c)}
   const exact=[...candidates.values()]; if(exact.length===1){const m=exact[0],stage='Orientation',volume=moneyNumber(cv(m.item,config.volumeColumnId)); matches.push({registration:r,mondayItemId:m.item.id,mondayName:m.item.name,stage,volume,joined:true,matchMethod:m.email&&m.phone?'Email + Phone':m.email?'Email':'Phone'});} else if(exact.length>1){possibleMatches.push({registration:r,reason:'Multiple Monday records match this email/phone',count:exact.length});}
     ...
     if(regName&&sameName.length===1&&sameNameRegs.length===1){
       ...
       matches.push({registration:r,mondayItemId:item.id,mondayName:item.name,stage:'Orientation',volume,joined:true,matchMethod:'Unique Exact Name'});
       diagnostics.push({candidateName:`${r.firstName||''} ${r.lastName||''}`.trim(),eventName:r.eventName||r.eventSlug||'Unknown event',registrationFound:true,mondayOrientationFound:true,emailMatches:Boolean(e&&me&&e===me),phoneMatches:Boolean(p&&mp&&p===mp),registrationHasEmail:Boolean(e),mondayHasEmail:Boolean(me),registrationHasPhone:Boolean(p),mondayHasPhone:Boolean(mp),note:'Matched by unique exact full name because email/phone differed.'});
     } else if(sameName.length){
       diagnostics.push({candidateName:`${r.firstName||''} ${r.lastName||''}`.trim(),eventName:r.eventName||r.eventSlug||'Unknown event',registrationFound:true,mondayOrientationFound:true,emailMatches:false,phoneMatches:false,registrationHasEmail:Boolean(e),mondayHasEmail:true,registrationHasPhone:Boolean(p),mondayHasPhone:true,note:'Name exists in Orientation but is not unique enough for automatic attribution.'});
     }
   }
 }
 ...
 return {configured:true,config,matches,possibleMatches,diagnostics,nameSourceDiagnostics,mondayItems:items.length,board:resolved.board,detected:resolved.detected,orientationGroup:{id:orientation.id,title:orientation.title}};
}
