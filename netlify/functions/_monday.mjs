...
export async function getRecruitingItems(board, columnIds=[]){
 const boardId=process.env.MONDAY_BOARD_ID; if(!boardId) throw new Error('MONDAY_BOARD_ID is not configured in Netlify.');
 ...
 for(const group of targetGroups){
   const groupId=String(group.id).replace(/\\/g,'\\\\').replace(/"/g,'\\"');
   ...
   let d=await mondayRequest(first,{ids:[String(boardId)],limit:100}); let page=d.boards?.[0]?.items_page;
   if(!page) continue;
   all.push(...(page.items||[])); let cursor=page.cursor;
   while(cursor){d=await mondayRequest(next,{cursor,limit:100});page=d.next_items_page;all.push(...(page?.items||[]));cursor=page?.cursor;}
 }
...
export async function getConversionData(registrations){
 ...
 const {items,orientation,activeRoster,groups}=await getRecruitingItems(resolved.board,[config.emailColumnId,config.phoneColumnId,config.volumeColumnId].filter(Boolean)); const emails=new Map(),phones=new Map();
