...
export default async req => {
  try {
    ...
    const url = new URL(req.url); const id = clean(url.searchParams.get("id"), 100);
    ...
    if (req.method === "POST") {
      ...
      const name = clean(body.name, 120); const email = clean(body.email, 180).toLowerCase();
      if (!name || !email) return json({ error: "Recruiter name and Bonzo email are required." }, 400);
      const now = new Date().toISOString();
      const recruiter = { id: crypto.randomUUID(), name, email, active: body.active !== false, createdAt: now, updatedAt: now };
    ...
    if (req.method === "PATCH" && id) {
      ...
      if (!current) return json({ error: "Recruiter not found." }, 404);
      const body = (await parseJSON(req)) || {};
      const updated = {
        ...current,
        name: body.name === undefined ? current.name : clean(body.name, 120),
        email: body.email === undefined ? current.email : clean(body.email, 180).toLowerCase(),
        active: body.active === undefined ? Boolean(current.active) : Boolean(body.active),
        updatedAt: new Date().toISOString()
      };
      if (!updated.name || !updated.email) return json({ error: "Recruiter name and Bonzo email are required." }, 400);
    ...
    return json({ error: "Unable to process recruiter request.", details: error.message }, 500);
  }
};
