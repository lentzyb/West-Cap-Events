...
export default async req => {
  try {
    ...
    if (req.method === "POST") {
      ...
      const calendlyUrl = clean(body.calendlyUrl, 500);
      if (calendlyUrl && !/^https:\/\/(?:www\.)?calendly\.com\//i.test(calendlyUrl)) return json({ error: "Calendly link must start with https://calendly.com/." }, 400);
      const recruiter = { id: crypto.randomUUID(), name, active: body.active !== false, calendlyUrl, showCalendly: body.showCalendly === true || body.showCalendly === "on", createdAt: now, updatedAt: now };
    ...
    if (req.method === "PATCH" && id) {
      ...
      const updated = {
        ...
        calendlyUrl: body.calendlyUrl === undefined ? clean(current.calendlyUrl, 500) : clean(body.calendlyUrl, 500),
        showCalendly: body.showCalendly === undefined ? Boolean(current.showCalendly) : Boolean(body.showCalendly),
        updatedAt: new Date().toISOString()
      };
      if (!updated.name) return json({ error: "Team member name is required." }, 400);
      if (updated.calendlyUrl && !/^https:\/\/(?:www\.)?calendly\.com\//i.test(updated.calendlyUrl)) return json({ error: "Calendly link must start with https://calendly.com/." }, 400);
